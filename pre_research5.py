...
import joblib
aozora_vector_seqs = joblib.load(open('/mnt/sdb/firebird555/aozora_vector_seqs.dat', 'rb'))
aozora_shifted_vector_seqs = joblib.load(open('/mnt/sdb/firebird555/aozora_shifted_vector_seqs.dat', 'rb'))
...
pre_decoder_outputs,_,_ = lstm_2(shift_with_context, initial_state=encoder_states)#pre_research4.pyとの違い
...
def decode_sequence(input_seq, time, random_state=False):
    ...
    if random_state:
        latent_vector = np.random.random(1,z_dim) #zにあたるもの
        ...
        z_mean, z_log_var, latent_vector = encoder_model.predict([input_seq, time])
...
for seq_index in range(100):
    ...
    input_sentence = ''
    for c in all_sentences[seq_index][:max_len]:
        input_sentence += c
    print('-')
